...
#[test]
fn test_new_oscillateur() {
    // Crée une nouvelle instance d'OscillateurSin
    let osc = OscillateurSin::new(440.0, 0.0, 44100.0, 10);
...
fn test_generate_and_get_next_sample() {
    let mut osc = OscillateurSin::new(440.0, 0.0, 44100.0, 10);

    // Génère un échantillon et vérifie qu'il est dans l'intervalle attendu
...
#[test]
fn test_generate_multiple_samples() {
    let mut osc = OscillateurSin::new(440.0, 0.0, 44100.0, 10);
    ...
    // Vérifie que le compteur d'échantillons est incrémenté correctement
    assert_eq!(osc.current_sample, 10);
}
